...
class Logger(object):
    ...
    def setup_monitoring(self, monitoring, monitoring_dir=None):
        ...
        if monitoring == 'telemetry':
            ...
            if self.tm.get_status() == 0:
                tqdm.write('Telemetry successfully connected.')
        elif monitoring == 'tensorboard':
            from torch.utils.tensorboard import SummaryWriter
            self.tb = SummaryWriter(monitoring_dir)
        else:
    ...
    def load_stats(self, filename):
        ...
        if not os.path.exists(filename):
            tqdm.write('Warning: file "%s" does not exist!' % filename)
            return
        ...
        try:
            ...
            tqdm.write('Warning: log file corrupted!')
